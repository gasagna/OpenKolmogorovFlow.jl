export LinearisedEquation, imex

abstract type AbstractMode end
abstract type AbstractDirectMode  <: AbstractMode end
abstract type AbstractAdjointMode <: AbstractMode end

# ~~~ Explicit Term of Linearised Equations ~~~
struct LinearisedExplicitTerm{n, M<:AbstractMode,        FT<:AbstractFTField,
                                 F<:AbstractField,       D1<:AbstractFTOperator, 
                                D2<:AbstractFTOperator, ITT<:InverseFFT!,
                               FTT<:ForwardFFT!, S, C}
    FTFStore::Vector{FT} # storage
      FStore::Vector{F}
       ...
       ifft!::ITT        # transforms
        ...
        cost::C          # forcing to linearised equation
end

# Outer constructor: TODO: merge with above
function LinearisedExplicitTerm(n::Int, m::Int, mode::AbstractMode, sol, cost, ::Type{S}, flags::UInt32) where {S}
    # stupidity check
    m ≥ n || throw(ArgumentError("`m` must be bigger than `n`, got `n, m= $n, $m`. Are you sure?"))

    # number of fields for storage
    nF  = mode isa AbstractAdjointMode ? 7 :
          mode isa AbstractDirectMode  ? 9 : error("wrong mode")

    # complex fields have size `n` but real fields might have larger size `m`
    FTFStore = [FTField(n, Complex{S}) for i = 1:nF]
    FStore   = [  Field(m, S)          for i = 1:nF-1]
...
(Eq::LinearisedExplicitTerm{n, FT})(t::Real, Λ::FT, dΛdt::FT, add::Bool=false) where {n, FT<:FTField{n}} = 
    Eq(t, Eq.sol(Eq.FTFStore[1], t), Λ, dΛdt)

# Adhere to callable interface for IMEXRKCB
function (Eq::LinearisedExplicitTerm{n, FT})(t::Real, Ω::FT, Λ::FT, dΛdt::FT, add::Bool=false) where {n, FT<:FTField{n}}
    ...
    mode isa AbstractDirectMode && U′,   V′   = Eq.FTFStore[8], Eq.FTFStore[9]
